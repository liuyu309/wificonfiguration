...
public class ConfigWireless {
	...
	private void checkRetry() throws Exception{
		...
		while (sendFlag)
		{
			...
			Log.i("ConfigWireless", "checkRetry configPacket.length="+configPacket.length);
			...
			for (int i = 0; i < configPacket.length; i++)
			{
				...
				for (int j = 0; j < 20; j++)
				{
					UdpClient.getInstance().sendBroadCastSignaling("239.1.2.110",bs);//send
	...
	private void sendData() throws Exception{
		while (sendFlagAp)
		{
			...
			Log.i("ConfigWireless", "sendData configPacket.length="+configPacket.length);

...
public class UdpConnector {
	...
	private DatagramPacket receivePacket = new DatagramPacket(receiveBuffer,1024);
...
//			wanReceiver();
	...
	public void sendBroadCasetBuffer(byte[] sendBuffer){
		if (isConnected()) {
			...
			//Log.i(tag,"send remaining len:" + buffer.remaining()+",content: "+strBuff);
			DatagramPacket sendPacket = new DatagramPacket(sendBuffer,sendBuffer.length );

				try{
					udpSocket.send(sendPacket);
					/*if(sendBuffer[3]!=(byte)0x01)
						Log.i(tag,"udpSocket send ip :"+sendPacket.getAddress().toString()
							+", port: "+sendPacket.getPort()+",bytes:"+strBuff.toString());*/
				} catch (IOException e) {
					e.printStackTrace();
				}
